...
<x-app-layout>
    ...
    @forelse ($questions as $question)
        <div class="w-half p-2 mt-6 mx-8 bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-md">
            <a href="{{ route('single.question', [ $question->subject, $question->id]) }}" class="text-l font-semibold p-8 py-3">{!! $question->content !!}</a>
        </div>
        @empty
        <div class="w-half p-2 mt-6 mx-8 w-half bg-white rounded-lg overflow-hidden">
            No question yet
        </div>
    @endforelse
