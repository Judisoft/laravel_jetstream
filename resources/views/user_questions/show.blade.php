        ...
        <div class="w-full bg-white  sm:max-w-2xl mt-6 p-5  shadow-md overflow-hidden sm:rounded-lg prose">
            ...
            @if(auth()->user()->is_premium == 1)
                @foreach ($user_question->answers as $user_answer)
                    <div class="flex flex-row mt-2 sm:items-center sm:justify-between">
                        <div class="text-center text-sm text-gray-500 sm:text-left">
                            <div class="flex items-center">
            ...
            @else
                ...
                <h1 class="text-gray-200">Upgrade your account to view answers</h1>
                <button href="{{ route('payment') }}" class="inline-flex items-center justify-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 focus:outline-none focus:border-red-700 focus:ring focus:ring-red-200 active:bg-red-600 disabled:opacity-25 transition">Upgrade account</button>
                </div>
            @endif
            @if ($errors->any())
                <div><hr>
            ...
            <form method="POST" action="{{ route('post.answer', $user_question->id) }}">
                ...
                <div class="px-4 py-5 sm:p-6 mx-auto">
                    <div class="col-span-4 sm:col-span-4">
                        ...
                        <textarea id="editor" name="answer"  rows="4" class="mt-1 block w-full" placeholder="Type answer here ..." autofocus>{{ old('answer') }}</textarea>
                    </div>
                </div>
